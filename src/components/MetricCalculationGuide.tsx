...
const METRICS: MetricDefinition[] = [
  ...
  },
  {
    ...
    cadence: "Daily",
    target: ">= 95%",
    source: "Weekly manual entry until request and response timestamps are supplied",
    formula: "Average of the weekly values entered inside the selected month. Each week is requests responded to within 1 hour / all requests x 100.",
    columns: ["Request timestamp", "First response timestamp"],
    note: "The current exports do not contain both timestamps, so this is recorded once per week. After a week is entered it is locked for non-admin editors; an admin can edit and confirm it.",
  },
  {
    label: "Safety",
    owner: "Drivers",
    ...
    formula: "Average of the weekly safety counts entered inside the selected month.",
    columns: [],
    note: "Record the weekly safety events, including speeding violations. The dashboard averages the weeks in the selected month and the weekly detail view shows the entries used.",
  },
  {
    ...
    cadence: "Monthly",
    target: "<= 10",
    source: "Weekly manual entry until the required fields are supplied",
    formula: "Average of the weekly incomplete-ticket counts entered inside the selected month.",
    columns: ["Labour Time", "Internal Notes"],
    note: "The current exports do not consistently provide these columns, so this is entered once per week. The weekly detail view shows the entries used in the monthly average.",
  },
  {
    ...
    columns: [],
    note: "Record one weekly total across these four job service quality categories. The dashboard averages the selected month's weekly values.",
  },
  {
